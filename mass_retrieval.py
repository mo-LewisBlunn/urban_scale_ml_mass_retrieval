...
def orog_ancil_retrieval(orography_ancil_mass_path, output_dir):
    ...
    if not os.path.exists(local_orog_path):
        ...
        print(f"Retrieving orography ancillary file:\n{' '.join(cmd)}")
        ...
        print(f"Orography file already exists locally at {local_orog_path}")
...
if __name__ == "__main__":
    ...
    orog_cube = orog_ancil_retrieval(config.orography_ancil_mass_path, config.grid_dir)
    target_grid = make_target_grid(orog_cube, config.target_grid_spacing, config.grid_dir, tol=1e-6)
